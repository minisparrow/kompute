...
void main()
{{
    uint col = gl_LocalInvocationID.x; // 0 .. tile_size
    uint row = gl_LocalInvocationID.y; // 0 .. tile_size
    // gl_WorkGroupID : 0 .. tensor_size / tile_size
    uint globalCol = {tile_size} * gl_WorkGroupID.x + col;
    uint globalRow = {tile_size} * gl_WorkGroupID.y + row;
    ...
    {{
        uint tiledRow = ({tile_size} * t) + row;
        uint tiledCol = ({tile_size} * t) + col;
        sub_tensor_1[row][col] = in_tensor_1[(globalRow * tensor_size) + tiledCol];
        sub_tensor_2[row][col] = in_tensor_2[(tiledRow * tensor_size) + globalCol];

        memoryBarrierShared();
        barrier();

        for(uint k = 0u; k < {tile_size}; k++)
            acc += sub_tensor_1[row][k] * sub_tensor_2[k][col];

        barrier();
    }}
    out_tensor[tensor_size * globalRow + globalCol] = acc;
}}
